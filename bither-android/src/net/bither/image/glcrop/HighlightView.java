...
class HighlightView {
	...
	private void init() {
		...
		mResizeDrawableWidth = resources
				.getDrawable(R.drawable.camera_crop_handle);
	...
	protected void draw(Canvas canvas) {
		...
		if (!hasFocus()) {
			...
			if (mCircle) {
				...
				mOutlinePaint.setColor(0xFFD8D8D8);
			}
	...
	void handleMotion(int edge, float dx, float dy) {
		...
		if (edge == GROW_NONE) {
			...
			if (yDelta == 0 && xDelta == 0)
				...
				delta = Math.min(yDelta, xDelta);
			}
			if (((edge & GROW_LEFT_EDGE) != 0) || ((edge & GROW_TOP_EDGE) != 0)) {
				delta = -delta;
			}
			growBy(delta, delta);
			// growBy((((edge & GROW_LEFT_EDGE) != 0) ? -1 : 1) * delta,
			// (((edge & GROW_TOP_EDGE) != 0) ? -1 : 1) * delta);
		}
	}
	...
	void growBy(float dx, float dy) {
		...
		r.inset(-dx, -dy);
		// Log.e(this.toString(),
		// "x:" + Float.toString(dx) + ",Y:" + Float.toString(dy));
		...
		// Log.e(this.toString(), "r,height:" + Float.toString(r.height())
		// + ",width:" + Float.toString(r.width()));
		if (Math.round(r.height()) != Math.round(r.width())) {
			return;
		}
		mCropRect.set(r);
		mDrawRect = computeLayout();
