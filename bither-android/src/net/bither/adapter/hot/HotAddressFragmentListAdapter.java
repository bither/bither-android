...
public class HotAddressFragmentListAdapter extends BaseExpandableListAdapter
	...
	public View getGroupView(final int groupPosition, boolean isExpanded,
		...
		if (convertView == null) {
			convertView = mLayoutInflater.inflate(
					R.layout.list_item_address_group, null);
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (convertView == null
				|| !(convertView instanceof AddressFragmentListItemView)) {
		...
		view.setOnClickListener(new AddressDetailClick(childPosition,
				isPrivate(groupPosition)));
		view.setOnLongClickListener(new AddressLongClick(childPosition,
				isPrivate(groupPosition)));
		return convertView;
	}
	...
	private class AddressLongClick implements OnLongClickListener {
		...
		public boolean onLongClick(View v) {
			Dialog dialog;
			if (isPrivate) {
				dialog = new DialogAddressWithPrivateKeyOption(activity,
						privates.get(position));
			} else {
				dialog = new DialogAddressWatchOnlyOption(activity,
						watchOnlys.get(position), null);
			}
			dialog.show();
			return true;
		}
	...
	public int getPinnedHeaderState(int groupPosition, int childPosition) {
		...
		if (childPosition == childCount - 1) {
			return PINNED_HEADER_PUSHED_UP;
				...
				&& !this.mListView.isGroupExpanded(groupPosition)) {
			return PINNED_HEADER_GONE;
		} else {
	...
	public void configurePinnedHeader(final View header,
			final int groupPosition, int childPosition, int alpha) {
