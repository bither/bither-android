...
public class OverScrollableViewPager extends ViewPager {
    ...
    protected boolean overScrollBy(int deltaX, int deltaY, int scrollX, int scrollY,
        ...
        if (newScrollX < 0) {
            maxOverScrollX = overScrollLeftWidth < 0 ? getWidth() / 4 : overScrollLeftWidth;
        } else if (newScrollX > scrollRangeX) {
            maxOverScrollX = overScrollRightWidth < 0 ? getWidth() / 4 : overScrollRightWidth;
        } else {
            maxOverScrollX = 0;
        }
        float overScrollRatio = 1;
        if (maxOverScrollX > 0 && isTouchEvent) {
            overScrollRatio = 1.0f - (float) Math.abs(newScrollX) / (float) maxOverScrollX;
            overScrollRatio = Math.abs(overScrollRatio);
        }
        return super.overScrollBy((int) (deltaX * overScrollRatio), deltaY, scrollX, scrollY,
