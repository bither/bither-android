...
public class DialogSendConfirm extends CenterDialog implements OnDismissListener {
    ...
    public DialogSendConfirm(Context context, SendRequest request, SendConfirmListener listener) {
        super(context);
        ...
        try {
            tvAddress.setText(GenericUtils.addressFromScriptPubKey(out.getScriptPubKey(),
                    MainNetParams.get()));
...
//		}
    ...
    private boolean isLowPriority(Transaction tx) {
        if (AppSharedPreference.getInstance().getTransactionFeeMode().ordinal() ==
                TransactionFeeMode.Low.ordinal()) {
