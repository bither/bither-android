...
public class TabButton extends FrameLayout implements OnShowListener,
		OnDismissListener {
	...
	private void init() {
		...
		addView(llIcon, new LayoutParams(LayoutParams.WRAP_CONTENT,
		...
		llIcon.addView(ivArrowDown, LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT);
		((LinearLayout.LayoutParams) ivArrowDown.getLayoutParams()).gravity = Gravity.CENTER_VERTICAL;
		LinearLayout.LayoutParams lpText = (LinearLayout.LayoutParams) tvText
				.getLayoutParams();
		lpText.weight = 1;
		lpText.width = 0;
		lpText.gravity = Gravity.CENTER_VERTICAL;
		lpText.leftMargin = UIUtil.dip2pix(-7);
		LayoutParams lpBottom = new LayoutParams(LayoutParams.MATCH_PARENT,
				UIUtil.dip2pix(2.67f));
		...
		tbtnBottom.setText("");
		tbtnBottom
				.setBackgroundResource(R.drawable.tab_bottom_background_selector);
		...
		tvText.getViewTreeObserver().addOnGlobalLayoutListener(
	...
	private void updateArrow() {
		...
		if (dialog != null && dialog.isShowing()) {
			...
			if (isChecked()) {
				...
				ivArrowDown
						.setImageResource(R.drawable.tab_arrow_down_unchecked);
			}
		}
	}
