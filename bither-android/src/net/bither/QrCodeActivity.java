...
public class QrCodeActivity extends SwipeRightFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent != null && intent.getExtras() != null) {
			codeString = intent.getExtras().getString(
					BitherSetting.INTENT_REF.QR_CODE_STRING);
		}
		if (StringUtil.isEmpty(codeString)) {
			finish();
			overridePendingTransition(0, 0);
		} else {
			contents = StringUtil.getQrCodeStringList(StringUtil
					.encodeQrCodeString(codeString));
			initView();
